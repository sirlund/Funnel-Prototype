<script>
	import { page } from '$app/stores';
	import { goto } from '$app/navigation';
	import { funnelStore } from '$lib/stores/funnel.js';
	import { formatCurrency } from '$lib/config/pricing.js';

	$: funnelId = $page.params.funnel;
	$: variant = $page.params.variant;
	$: pricing = $funnelStore.pricing;
	$: sport = $funnelStore.answers.sport;

	function goBack() {
		if (typeof window !== 'undefined') {
			window.history.back();
		}
	}

	function selectPlan(planType) {
		// Store selected plan and navigate to summary
		funnelStore.saveAnswer('selectedPlan', planType);
		goto(`/${funnelId}/${variant}/summary`);
	}
</script>

<svelte:head>
	<title>Recomendación - Klare</title>
</svelte:head>

<div class="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 py-12 px-4">
	<div class="max-w-6xl mx-auto">
		<header class="text-center mb-8">
			<a href="/">
				<svg
					width="50"
					height="20"
					viewBox="0 0 50 20"
					class="mx-auto mb-4"
					xmlns="http://www.w3.org/2000/svg"
					xmlns:xlink="http://www.w3.org/1999/xlink"
				>
					<defs>
						<path
							d="M21.442 13.283c-1.787.367-1.79 1.503-1.79 1.924 0 .79.4 2.116 3.071 2.116 1.757 0 3.07-1.197 3.07-2.267v-1.478c.001 0-1.671-.949-4.35-.295zm6.658 6.122h-2.306v-1.342c-1.188 1.635-3.768 1.497-4.078 1.474-2.685-.322-4.368-1.948-4.368-4.326 0-2.704 2.01-4.312 5.375-4.312 1.261 0 2.268.14 3.071.423v-.812c.052-1.57-.565-2.015-.769-2.163-1.29-.931-4.104-.262-4.988.045l-.772-2.14a13.512 13.512 0 0 1 2.482-.565c1.965-.252 3.523.025 4.635.824.822.591 1.786 1.774 1.716 4.056l.002 8.838zm-17.558-.014L5.744 11.2l4.423-3.942-1.545-1.684-6.317 5.629V.154H0V19.41h2.305v-5.147l1.691-1.508 3.885 6.632 2.661.003zm33.23-11.596c1.6 0 2.696.6 3.348 1.835.155.297.278.61.368.934h-7.544c.455-1.428 1.495-2.77 3.797-2.77h.03zm4.052 6.888c-1.048 1.87-2.415 2.794-4.064 2.74h-.123c-1.183 0-2.08-.325-2.742-.99-1.081-1.098-1.251-2.835-1.277-3.439H50l-.014-1.154a7.396 7.396 0 0 0-.779-3.015c-.73-1.436-2.266-3.147-5.439-3.147-1.612 0-2.755.492-3.437.906-2.38 1.448-2.954 4.126-3.017 6.117-.005.131-.086 3.255 1.919 5.297 1.103 1.125 2.586 1.694 4.403 1.694h.254c2.425 0 4.533-1.383 5.953-3.915l-2.019-1.094zM37.692 5.782l-.234-.04c-2.269-.356-4.082-.01-5.242.998-.999.868-1.48 2.154-1.428 3.823v8.84h2.304V10.51c-.031-.968.187-1.663.65-2.065.592-.514 1.71-.685 3.163-.485l.787-2.177zM15.238.154h-2.305v19.25h2.305V.155zm-14.062 0h.046-.046z"
							id="logo-path"
						></path>
					</defs>
					<g fill="none" fill-rule="evenodd">
						<mask id="logo-mask" fill="#fff"><use xlink:href="#logo-path"></use></mask>
						<g mask="url(#logo-mask)" fill="#1E40AF"><path d="M0 0h50v20H0z"></path></g>
					</g>
				</svg>
			</a>
		</header>

		<div class="text-center mb-12">
			<h1 class="text-3xl font-bold text-gray-900 mb-2">
				Seguro para <span class="text-primary">{sport || 'tu deporte'}</span>
			</h1>
			<p class="text-gray-600">Selecciona el plan que mejor se ajusta a tus necesidades</p>
		</div>

		{#if pricing}
			<div class="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
				<!-- Base Plan -->
				<div class="card hover:shadow-2xl transition-all transform hover:-translate-y-1">
					<div class="bg-primary text-white px-4 py-2 rounded-t-lg -mx-6 -mt-6 mb-6">
						<h3 class="text-lg font-semibold text-center">Más ajustado a ti</h3>
					</div>

					<div class="text-center mb-6">
						<p class="text-gray-600 mb-4">{pricing.base.displayDays} asegurado por</p>
						<div class="text-5xl font-bold text-gray-900 mb-2">
							${formatCurrency(pricing.base.totalPrice)}
						</div>
						<p class="text-gray-500">${formatCurrency(pricing.base.pricePerDay)} por día</p>
					</div>

					<div class="space-y-3 mb-8">
						<div class="text-gray-700">
							Reembolso de gastos médicos para 8 lesiones.
						</div>
						<h4 class="font-semibold text-gray-900">Además incluye:</h4>
						<ul class="space-y-2 text-gray-700">
							<li class="flex items-center">
								<svg
									class="w-5 h-5 text-green-500 mr-2"
									fill="currentColor"
									viewBox="0 0 20 20"
								>
									<path
										fill-rule="evenodd"
										d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
										clip-rule="evenodd"
									/>
								</svg>
								Cobertura médica
							</li>
							<li class="flex items-center">
								<svg
									class="w-5 h-5 text-green-500 mr-2"
									fill="currentColor"
									viewBox="0 0 20 20"
								>
									<path
										fill-rule="evenodd"
										d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
										clip-rule="evenodd"
									/>
								</svg>
								Asistencia 24/7
							</li>
						</ul>
						<p class="text-xs text-gray-500 mt-4">Cobertura entregada por Consorcio</p>
					</div>

					<button on:click={() => selectPlan('base')} class="btn btn-primary w-full">
						Configurar este seguro
					</button>
				</div>

				<!-- Alternative Plan (only show if applicable) -->
				{#if pricing.showAlternative}
					<div class="card hover:shadow-2xl transition-all transform hover:-translate-y-1">
						<div class="bg-secondary text-white px-4 py-2 rounded-t-lg -mx-6 -mt-6 mb-6">
							<h3 class="text-lg font-semibold text-center">Más conveniente</h3>
						</div>

						<div class="text-center mb-6">
							<p class="text-gray-600 mb-4">{pricing.alternative.displayDays} asegurado por</p>
							<div class="text-5xl font-bold text-gray-900 mb-2">
								${formatCurrency(pricing.alternative.totalPrice)}
							</div>
							<p class="text-gray-500">
								${formatCurrency(pricing.alternative.pricePerDay)} por día
							</p>
						</div>

						<div class="space-y-3 mb-8">
							<div class="text-gray-700">
								Reembolso de gastos médicos para todas las lesiones.
							</div>
							<h4 class="font-semibold text-gray-900">Además incluye:</h4>
							<ul class="space-y-2 text-gray-700">
								<li class="flex items-center">
									<svg
										class="w-5 h-5 text-green-500 mr-2"
										fill="currentColor"
										viewBox="0 0 20 20"
									>
										<path
											fill-rule="evenodd"
											d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
											clip-rule="evenodd"
										/>
									</svg>
									Cobertura médica completa
								</li>
								<li class="flex items-center">
									<svg
										class="w-5 h-5 text-green-500 mr-2"
										fill="currentColor"
										viewBox="0 0 20 20"
									>
										<path
											fill-rule="evenodd"
											d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
											clip-rule="evenodd"
										/>
									</svg>
									Asistencia 24/7
								</li>
							</ul>
							<p class="text-xs text-gray-500 mt-4">Cobertura entregada por Consorcio</p>
						</div>

						<button on:click={() => selectPlan('alternative')} class="btn btn-primary w-full">
							Configurar este seguro
						</button>
					</div>
				{/if}
			</div>
		{:else}
			<div class="card text-center">
				<p class="text-gray-600">Calculando tu recomendación...</p>
			</div>
		{/if}

		<div class="text-center mt-8">
			<button type="button" on:click={goBack} class="btn btn-secondary">
				← Volver
			</button>
		</div>
	</div>
</div>
